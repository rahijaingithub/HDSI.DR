...
data_fit =  function(datatype = c("simulated", "real"), param = list(miss = 33, corr = 0.9, cutter = 10, seed =1, data_code=1, test_train_ratio=0.2, varnum = 15, setting= "Correlation", main_var=10, var_effect=c(0.5, -0.5), correlation_var=15, correlation_val=5, high_dim=T, train_sample=500, var = "Mar")){
  if(datatype == "real"){
    ...
    df_p=dataset(varnum = param$varnum, setting= param$setting, var= param$var, main_var=param$main_var, var_effect=param$var_effect, correlation_var=param$correlation_var, correlation_val=param$correlation_val, high_dim=param$high_dim, train_sample=param$train_sample)
  }
  {
    # Generate Dataset
