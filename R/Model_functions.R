...
predict_metric=function(actualy=validationdf[,outname[x]], predictedy=y_estimate){
  ...
  res=data.frame(Corr=Corr, RMSE=RMSE, rsquare=rsquare, stringsAsFactors = F)
  return(res)
}
  ...
  if(length(varlist)>=1){
    if(!is.na(varlist)){
      ...
      Perf=tryCatch({predict_metric(actualy = testdf$y, predictedy = y_est)},
                    error=function(e){print("Error"); data.frame(Corr=0, RMSE = sqrt(MLmetrics::MSE(testdf$y,mean(testdf$y, na.rm=T))), rsquare=0, stringsAsFactors = F)})
    }
    else{
      Perf = data.frame(Corr=0, RMSE = sqrt(MLmetrics::MSE(testdf$y,mean(testdf$y, na.rm=T))), rsquare = 0, stringsAsFactors = F)}
  }
  else{Perf = data.frame(Corr=0, RMSE = sqrt(MLmetrics::MSE(testdf$y,mean(testdf$y, na.rm=T))), rsquare = 0, stringsAsFactors = F)}
  # print(Perf)
