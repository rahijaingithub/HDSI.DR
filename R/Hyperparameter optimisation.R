...
optim_hyperpara = function(optimtype = c("GA", "Grid"), cv= T, df_p, k = seq(5,varnum-1), ncomp= seq(2,10), summary_ci = seq(0.0, 0.99, 0.01), varmax =  seq(-2000, 2000), otherpara){
  ...
  if(optimtype == "GA"){
    ...
    # print(nbits)
    set.seed(3)
    GA <- GA::ga(type = "binary", fitness = GA_OF, nBits = nbits, popSize = 10, maxiter = 150, run=10, pcrossover = 0.8, pmutation = 0.5, parallel = F, monitor = T) #, suggestions = c(GA::decimal2binary(62703,nbits))
